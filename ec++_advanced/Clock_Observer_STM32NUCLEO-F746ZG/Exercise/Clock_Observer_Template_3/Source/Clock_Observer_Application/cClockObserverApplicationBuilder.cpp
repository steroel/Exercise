...
namespace Clock_Observer_Application
{
	cClockObserverApplicationBuilder::cClockObserverApplicationBuilder(void) :
	  mClock{ 23, 59, 50 },
	  mAnalogClock{ 23, 59, 49 },
	  mDigitalClock{ 23, 59, 48 },
	  mAnalogDigitalClock{ 23, 59, 47, 23, 59, 46 }
	...
	void cClockObserverApplicationBuilder::execute(void)
	{
		...
		while(true)
		{
			...
			switch (getCommand())
			{
				...
				case 'D':
					mDigitalClock.update();
					mDigitalClock.show();
				break;
				
				case 'E':
					mAnalogDigitalClock.update();
					mAnalogDigitalClock.show();
				break;

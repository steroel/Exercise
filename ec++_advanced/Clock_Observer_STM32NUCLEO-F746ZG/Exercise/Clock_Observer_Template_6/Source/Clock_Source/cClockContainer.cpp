...
{
	void cClockContainer::attach(icObserver* const ptrObserver)
	{
	...
	void cClockContainer::detach(icObserver* const ptrObserver)
	{
	...
	void cClockContainer::notify(void)
	{
		...
		*/
		...
		for_each(mObserverPtrList.begin(), mObserverPtrList.end(),
				[](icObserver* const ptrObserver){ ptrObserver->update();
				                                   ptrObserver->show(); });

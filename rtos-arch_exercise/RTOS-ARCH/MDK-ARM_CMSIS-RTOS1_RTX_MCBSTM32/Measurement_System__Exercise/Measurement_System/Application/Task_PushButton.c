...
//	if (... == osOK)	// request the display as resource
//	{																												// successful
		LCD_printStringXY(15, 0, "_");												// access display resource
//		if (... != osOK)						// release display resource
//		{
...
//	if (... == osOK)	// request the display resource
//	{																												// successful
...
{
//	...;								// set signal to push button task
}
...
//	timerHandle_PushButton = ...;		// create interval timer
	
	if (timerHandle_PushButton != NULL)												// interval timer creation successful
	{
//		if (... == osOK)	// start interval timer
		{
			while(1)
			{

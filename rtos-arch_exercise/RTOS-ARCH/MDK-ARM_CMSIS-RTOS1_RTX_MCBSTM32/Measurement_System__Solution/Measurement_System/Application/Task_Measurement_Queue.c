...
void Task_Measurement_Queue(void const *argument)
{
  ...
  {
	  //osDelay(1000);
		locEventADC_Message = osMailGet(queueHandle_ADC_Measurement, 500);					// wait for ADC message
		...
		{		
			...
			locEventCommandMessage = osMailGet(queueHandle_Command_Measurement, 0);		// read command message without waiting
...
void ADC1_2_IRQHandler(void) __irq																					// ADC interrupt service routine
{
	...
	if (locPtrSendMessage != NULL)
	{
		...
		if (osMailPut(queueHandle_ADC_Measurement, locPtrSendMessage) != osOK)	// send message with ADC result to task
		{
	...
	LED_toggle(LED5);	// for measurement purpose only
}

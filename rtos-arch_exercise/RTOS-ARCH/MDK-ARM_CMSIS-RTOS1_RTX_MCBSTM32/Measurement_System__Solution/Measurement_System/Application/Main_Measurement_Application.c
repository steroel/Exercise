...
int32_t main (void)
{
  ...
  if (osKernelInitialize() != osOK)	// initialize operating system kernel
	{																	// failed
		printf("Fatal error main: kernal initialization failed\n");
		while(1);
	}
		
	locStatusCode = Task_Init();			// create all operating system objects
  ...
  if (osKernelStart() != osOK) 			// start scheduling thread execution
	{																	// failed
